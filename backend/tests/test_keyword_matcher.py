...
def test_skill_present_in_both_job_and_cv_is_met():
    ...
    result = matcher.match(job_text=job_text, cv_text=cv_text, output_language="tr")

    assert result.job_title is None
    assert result.company_name is None
    by_requirement = {m.requirement_text: m for m in result.matches}
...
def test_skill_not_mentioned_in_job_is_not_a_requirement():
    ...
    assert {m.requirement_text for m in result.matches} == {"Python"}
...
def test_cross_language_alias_matching():
    ...
    result = matcher.match(job_text=job_text, cv_text=cv_text, output_language="en")

    ml_match = next(m for m in result.matches if m.requirement_text == "Machine Learning")
...
def test_turkish_capital_i_alias_matches():
    ...
    english_match = next(m for m in result.matches if m.requirement_text == "English")
...
def test_explanation_language_follows_output_language():
    ...
    tr_matches = matcher.match(job_text=job_text, cv_text=cv_text, output_language="tr").matches
    en_matches = matcher.match(job_text=job_text, cv_text=cv_text, output_language="en").matches
